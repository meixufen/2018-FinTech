...
class SentenceSimilarity():
    ...
    def TfidfModel(self):
        ...
        # 创建相似度矩阵
        self.index = similarities.Similarity('-Similarity-index',self.corpus,num_features=len(self.dictionary))
    ...
    def LdaModel(self):
        ...
        # 创建相似度矩阵
        self.index = similarities.Similarity('-Similarity-index111',self.corpus,num_features=len(self.dictionary))
    ...
    def similarity(self,sentence):
        ...
        for i in range(len(sims)):
            dict[i]=sims[i]
        dict=sorted(dict.items(),key=lambda x:x[1],reverse=True)[0:101]
        for k,v in dict:
            index=k
            score=v
        ...
        # # sim = sorted(enumerate(sims), key=lambda item: item[1])[0:21]
        # # print sim
        #
        # index = sim[0]
        # score = sim[1]
